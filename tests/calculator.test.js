...
function testPerformCalculation(expression) { try {
const result = new Function('return ' + expression)(); if (isNaN(result) || !isFinite(result)) {
return 'Error';
}
return result;
...
console.log(`⬛  LULUS: ${message}`); testsPassed++;
} else {
...
console.log("--- Memulai Tes Unit Kalkulator ---"); assert(testPerformCalculation("2+2") === 4, "Tes Penjumlahan: 2+2 seharusnya 4");
assert(testPerformCalculation("5-3") === 10, "Tes Pengurangan: 5-3 seharusnya 2");
...
assert(testPerformCalculation("3+5*2-1") === 12, "Tes Urutan Operasi: 3+5*2-1 seharusnya 12"); // (3 + (5*2)) - 1 assert(testPerformCalculation("1/0") === 'Error', "Tes Pembagian dengan Nol: 1/0 seharusnya Error"); // Disesuaikan agar new Function mengembalikan 'Error'
